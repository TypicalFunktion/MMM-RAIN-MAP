...
2. Add the module configuration into the `MagicMirror/config/config.js` file (sample configuration):
   ...
   ```
      ...
      **Important for US Users:** For map locations within the United States, use `mapUrl: "https://tile.openstreetmap.org/{z}/{x}/{y}.png"` instead of the default German tile server.

   **Getting Coordinates:** For `mapPositions`, use the latitude and longitude values for your location. You can get these coordinates from the address bar when viewing your location on https://www.openstreetmap.org/ - the URL will contain the lat/lng values.
     }
    }
    ```
